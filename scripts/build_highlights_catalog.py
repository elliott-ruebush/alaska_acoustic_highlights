...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--spectrogram-thumbs-dir",
        type=Path,
        default=DEFAULT_SPECTROGRAM_THUMBS,
        help=(
            f"Gallery spectrogram thumb root mirroring audio layout "
            f"(default: {DEFAULT_SPECTROGRAM_THUMBS.relative_to(PROJECT_ROOT)})"
        ),
    )
    parser.add_argument(
        "--catalog",
...
def spectrogram_thumb_path(
    audio_path: Path,
    input_root: Path,
    thumbs_root: Path,
) -> str:
    rel_audio = audio_path.relative_to(input_root)
    thumb_rel = rel_audio.with_suffix(".webp")
    thumb_file = thumbs_root / thumb_rel
    return repo_relative(thumb_file)


def load_existing_site_photo_ids(output_path: Path) -> dict[str, str | None]:
    if not output_path.is_file():
        return {}
    with output_path.open(encoding="utf-8") as handle:
        catalog = json.load(handle)
    return {
        clip["id"]: clip.get("site_photo_id")
        for clip in catalog
        if "id" in clip
    }


def merge_site_photo_ids(
    clips: list[dict[str, Any]],
    existing_ids: dict[str, str | None],
) -> None:
    for clip in clips:
        existing = existing_ids.get(clip["id"])
        if existing is not None:
            clip["site_photo_id"] = existing


def assign_site_photos_from_catalog(clips: list[dict[str, Any]]) -> int:
    photos = load_site_photos_catalog(CATALOG_SITE_PHOTOS)
    if not photos:
        return 0
    assign_site_photo_ids(clips, photos)
    return sum(1 for clip in clips if clip.get("site_photo_id"))
...
def build_clip(
    ...
    spectrogram_thumb_repo_path = spectrogram_thumb_path(
        path, input_root, spectrogram_thumbs_root
    )
    ...
    return {
        ...
        "spectrogram_thumb_path": spectrogram_thumb_repo_path,
...
def summarize_spectrograms(
    clips: list[dict[str, Any]],
    *,
    path_field: str = "spectrogram_path",
    ...
    for clip in clips:
        spec_path = PROJECT_ROOT / clip[path_field]
        expected += 1
        if spec_path.is_file():
            present += 1
        else:
            missing.append(clip[path_field])
...
def print_summary(
    ...
    thumb_present, thumb_expected, _ = summarize_spectrograms(
        clips, path_field="spectrogram_thumb_path"
    )
    print(f"Spectrogram thumbs present: {thumb_present}/{thumb_expected}")

    site_photo_count = sum(1 for clip in clips if clip.get("site_photo_id"))
    print(f"Clips with site photos: {site_photo_count}/{len(clips)}")
    print(
        "When adding NEW site photos, run: "
        "python scripts/build_site_photos.py --sync-catalog --execute"
    )
    ...
    if missing_thumbs:
        print(f"Missing spectrogram thumb files ({len(missing_thumbs)}):")
        for path in missing_thumbs[:20]:
            print(f"  - {path}")
...
def main(argv: list[str] | None = None) -> int:
    ...
    spectrogram_thumbs_root = args.spectrogram_thumbs_dir.resolve()
    ...
    existing_site_photo_ids = load_existing_site_photo_ids(output_path)
    ...
    clips = [
        build_clip(
            ...
            input_root,
            spectrograms_root,
            spectrogram_thumbs_root,
            catalog_index,
            used_ids,
        )
        for path in audio_files
    ]
    merge_site_photo_ids(clips, existing_site_photo_ids)
    assign_site_photos_from_catalog(clips)
    ...
    _, _, missing_thumbs = summarize_spectrograms(clips, path_field="spectrogram_thumb_path")
    print_summary(clips, missing_specs, missing_thumbs)
