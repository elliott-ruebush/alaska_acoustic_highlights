...
| `build_highlights_catalog.py` | Build site-facing JSON from `highlights/audio/` (+ spectrograms, CSV enrichment, site names; reads MP3 tags for enrichment) → `data/catalog/highlights.json` |
| `validate_highlights_catalog.py` | CI/catalog gate: JSON Schema + media paths + `site_photo_id` refs; exit 1 on failure |
| `build_site_names.py` | Build `data/catalog/site_names.csv` from `Complete_Metadata_AKR_2001-2025.xlsx` |
| `build_site_photos.py` | Copy cardinal site photos from external drive (or manual drops); maintain `data/catalog/site_photos.json`; assign `site_photo_id` on clips |
| `generate_highlights_spectrograms.py` | Batch log-frequency spectrogram PNGs + gallery WebP thumbs; mirror `highlights/audio/` → `highlights/spectrograms/` and `highlights/spectrograms_thumbs/` |
...
| `trim_clip_range.py` | Manual time-range trim of one clip; backs up to `archive/pre_trim/`; updates catalog `duration_sec` after trim |
...
- **Catalog:** `build_catalog`, `build_highlights_catalog`, `build_site_photos`, `validate_highlights_catalog`
...
| Python packages | `pandas`, `librosa`, `soundfile`, `mutagen`, `matplotlib`, `numpy`, `PIL` (Pillow), `jsonschema`, `openpyxl` |
...
| `build_highlights_catalog.py` | `--input`, `--output`, `--spectrograms-dir`, `--spectrogram-thumbs-dir`, `--catalog` |
| `generate_highlights_spectrograms.py` | `--input`, `--output`, `--thumb-output`, `--force`, `--dry-run`, `--thumbs-only`, `--limit`, `--files` |
...
highlights/spectrograms_thumbs/     generate_highlights_spectrograms.py (--thumbs-only to rebuild thumbs from PNGs)
highlights/site_photos/             build_site_photos.py
        ↓
data/catalog/site_photos.json       build_site_photos.py
data/catalog/highlights.json        build_highlights_catalog.py (+ site photo assignment)
        ↓
site/src/lib/catalog.ts             reads ../data/catalog/highlights.json + site_photos.json
...
2. **Refresh site catalog after audio changes:** `build_highlights_catalog.py` (re-run `build_site_photos.py --sync-catalog --execute` if photos changed).
